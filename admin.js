/* ─────────────────────────────────────────────
   NICE WRITING — DASHBOARD
   (auth is handled by dashboard.html inline script)
───────────────────────────────────────────── */
...
function openEditModal(id) {
  ...
  if (item.file_url) {
    setAttachMode('file');
    document.getElementById('current-file-note').textContent = `Current file: ${item.file_name || 'attached'}`;
  } else {
    setAttachMode('link');
  }
  openModal();
...
async function saveWork() {
  ...
  const { error } = editingId
    ? await sb.from('work_items').update(payload).eq('id', editingId)
